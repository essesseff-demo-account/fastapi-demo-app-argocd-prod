# fastapi-demo-app - Argo CD Application (PROD)

This repository contains the Argo CD Application manifest for the **PROD** environment of the fastapi-demo-app essesseff™ app.  
...
```
fastapi-demo-app-argocd-prod/
├── app-of-apps.yaml.template                      # Root Application template
├── argocd/
│   └── fastapi-demo-app-prod-application.yaml.template # PROD environment Application manifest (auto-synced) template
...
│   └── offboard-fastapi-demo-app-prod.sh                  # script for offboarding the fastapi-demo-app prod app 1) from essesseff only or 2) from Argo CD and K8s entirely
├── setup-argocd-cluster.sh           # Argo CD K8s setup script 
├── setup-argocd.sh                   # Argo CD fastapi-demo-app-prod essesseff app setup script 
└── README.md                         # This file
```
...
### Configure fastapi-demo-app-prod essesseff App to Argo CD and deploy to K8s

***Note: it is highly recommended that, rather than going through all the steps here to configure Argo CD, that for convenience and faster turnaround time you instead use the [essesseff-onboarding-utility](https://github.com/essesseff/essesseff-onboarding-utility)***

1. **Git Clone This Repository**:
   ```bash
   git clone git@github.com:essesseff-demo-account/fastapi-demo-app-argocd-prod.git
   ```
...
2. **Configure Environment Variables in .env File**:
   ...
   Then set the environment variables in the .env which will be used for generating the following from the templates in this repository:
      ...
      - `fastapi-demo-app-argocd-prod` repository (to read Application manifests)
      - `fastapi-demo-app-config-prod` repository (to read Helm charts and values)
      ...
      c. ***Configuration of fastapi-demo-app-prod Deployment in Argo CD***:

      fastapi-demo-app-prod-application.yaml is used to configure the fastapi-demo-app-prod deployment

      d. ***Configuration of fastapi-demo-app-argocd-prod App-of-Apps Deployment in Argo CD***:

      app-of-apps.yaml is used to configure the fastapi-demo-app-argocd-prod app-of-apps deployment

      e. ***Configuration of fastapi-demo-app-prod Argo CD Notifications to essesseff***:

      If notifications-secret.yaml is downloaded from essesseff for fastapi-demo-app-prod, notifications-configmap.yaml will be used to configure Argo CD notifications to essesseff.
...
3. **(if an essesseff-subscribed app) Configure Argo CD Notifications Secrets**:
   ...
   Request the notifications-secret.yaml file contents from the essesseff UX for fastapi-demo-app here:
   https://www.essesseff.com/home/[YOUR_essesseff_TEAM_ACCOUNT]/apps/fastapi-demo-app/settings
...
4. **Run the setup-argocd.sh Script**:
   ...
   This script applies all secrets, configmaps, Argo CD application definitions, etc. for fastapi-demo-app PROD.
...
5. **Verify in Argo CD UI**:
   ...
   You should see:
   - `fastapi-demo-app-argocd-prod` - Root Application (watches this repository)
   - `fastapi-demo-app-prod` - Environment Application (auto-synced by root Application)

6. **Access the Deployed Application**:
   ```bash
   kubectl port-forward service/fastapi-demo-app-prod 8081:80 -n essesseff-demo-account
   # Access: http://localhost:8081
   ```
### How to Offboard fastapi-demo-app-prod Deployment from Argo CD and K8s

1. **Execute the offboarding script**:
   ```bash
   cd offboarding
   chmod 744 offboard-fastapi-demo-app-prod.sh
   ./offboard-fastapi-demo-app-prod.sh
   ```
...
- **Name**: `fastapi-demo-app-prod`
- **Namespace**: `argocd`
- **Source Repository**: `fastapi-demo-app-config-prod`
...
3. essesseff GitHub App automation triggers essesseff to auto-update Helm `fastapi-demo-app-config-prod/Chart.yaml` and `fastapi-demo-app-config-prod/values.yaml` with the image tag of the newly built image
4. Argo CD syncs PROD Application automatically on K8s

### Manual GitOps Deployment

1. Push update(s) to the `main` branch in `fastapi-demo-app-config-prod` repository, typically to Helm Chart.yaml and/or values.yaml
2. Argo CD syncs PROD Application automatically on K8s

## Repository URLs

- **Source**: `https://github.com/essesseff-demo-account/fastapi-demo-app`
- **Config PROD**: `https://github.com/essesseff-demo-account/fastapi-demo-app-config-prod`
- **Argo CD PROD**: `https://github.com/essesseff-demo-account/fastapi-demo-app-argocd-prod` (this repo)
...
- Allow Argo CD to detect changes in `argocd/fastapi-demo-app-prod-application.yaml` more quickly
